...
module.exports = {
    ...
    doLogin: (userData) => {
        return new Promise(async (resolve, reject) => {
            ...
            if (user.blockStatus === true)   //Checking whether the user iis blockked or not................
            {
    ...
    validation:
        [
            ...
            check('email', 'Email is required')
                .isEmail()
                .normalizeEmail(),
            check('mobilenumber', 'Mobile number should contains 10 digits')
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (userCart) {
                ...
                db.get().collection(collection.CART_COLLECTION).insertOne(cartObj).then((response) =>
                 {
                        db.get().collection(collection.WISHLIST_COLLECTION).deleteOne({user: objectId(userId), 'products.item' : objectId(proId)}).then((response)=>
                        {
                            resolve()
                        })
    ...
    getUserOrders: (userId) => {
        return new Promise(async (resolve, reject) => {
            ...
            let orders = await db.get().collection(collection.ORDER_COLLECTION).find({ "products.user": objectId(userId) }).toArray()
    ...
    deleteOrders: (userId) => {
        return new Promise((resolve, reject) => {
            db.get().collection(collection.ORDER_COLLECTION).deleteOne({ _id: objectId(userId) }).then((response) => {
                resolve(response)
            })
        })
    },
    ...
    getAddress: (userId) => {
        return new Promise(async (resolve, reject) => {
            await db.get().collection(collection.USER_COLLECTION).findOne({ _id: objectId(userId) }).then((address) => {
                resolve(address)
            })
        })
    },
    ...
    updateOrderStatus: (details) => {
        return new Promise(async (resolve, reject) => {
            await db.get().collection(collection.ORDER_COLLECTION).updateOne({ _id: objectId(details.orderId) },           // C H A N G E   H E R E . . . ..
                {
                    $set:
                    {
    ...
    catProducts: (id) => {
        return new Promise((resolve, reject) => {
            db.get().collection(collection.PRODUCT_COLLECTION).find({ category: objectId(id) }).toArray().then((product) => 
            {
                resolve(product)
            })
        })
